    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="p-0 gap-0 flex flex-col w-screen h-[100dvh] max-w-none top-0 left-0 translate-x-0 translate-y-0 rounded-none border-none overflow-hidden z-[100]">
                ...
                <DialogFooter className="p-4 border-t bg-background flex-shrink-0 flex flex-row gap-4 justify-between sm:justify-between items-center sticky bottom-0 w-full">
                    <Button variant="outline" onClick={() => setSignature(null)} className="flex-1 font-headline h-12">
                        <Eraser className="mr-2 h-4 w-4" /> Clear
                    </Button>
                    <Button onClick={handleSave} className="flex-1 font-headline h-12">
