...
import { Loader2, Package, User, Globe, Search, X, Check } from "lucide-react"
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl p-0 overflow-hidden shadow-2xl flex flex-col max-h-[95vh]">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-3">
                      <FormLabel className="font-headline text-xs uppercase tracking-widest text-muted-foreground">Allocation Target</FormLabel>
                      <FormControl>
                        <RadioGroup
                ...
                {assignmentType === 'individual' && (
                  <div className="space-y-4 border-2 rounded-2xl p-5 bg-muted/20 border-dashed animate-in fade-in duration-300">
                    <FormLabel className="font-headline text-xs uppercase tracking-wider text-primary">Search Personnel Directory</FormLabel>
                    ...
                    {selectedUserId ? (
                      <div className="flex items-center justify-between p-4 bg-primary/10 border-2 border-primary/20 rounded-xl">
                          <div className="flex flex-col">
                              ...
                              <span className="text-[10px] font-black uppercase text-primary tracking-widest">{profiles[selectedUserId]?.code || "PMR"} • {profiles[selectedUserId]?.email}</span>
                          </div>
                          <Button 
                              variant="ghost" 
                              size="icon" 
                      ...
                      <div className="space-y-3">
                          <div className="relative">
                              ...
                              <Input 
                                  ...
                                  className="pl-10 h-11 border-2 focus-visible:ring-primary rounded-xl bg-background"
                              />
                          </div>
                          <div className="max-h-[300px] overflow-y-auto border-2 rounded-xl bg-background divide-y shadow-inner scrollbar-hide">
                              {filteredUsers.length > 0 ? (
                                  ...
                                  <div className="p-8 text-center text-sm text-muted-foreground italic">No matching field personnel found.</div>
                              )}
                          </div>
                      </div>
                    )}
                ...
                <div className="grid grid-cols-1 gap-5 pt-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                            <FormLabel className="font-headline text-xs uppercase tracking-wider">Product Category</FormLabel>
                            <FormControl>
                                <Input placeholder="e.g. Antihistamine" {...field} className="h-11 border-2 rounded-xl bg-background" />
                            </FormControl>
                            <FormMessage />
                            </FormItem>
                        )}
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                            <FormLabel className="font-headline text-xs uppercase tracking-wider">Material Name</FormLabel>
                            <FormControl>
                                <Input placeholder="e.g. PQ3_Frutos" {...field} className="h-11 border-2 rounded-xl bg-background" />
                            </FormControl>
                            <FormMessage />
                            </FormItem>
                        )}
                    />
                     <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-headline text-xs uppercase tracking-wider">Bag Quantity</FormLabel>
                          <FormControl>
                            <Input type="number" {...field} className="h-14 text-3xl font-mono border-2 rounded-xl bg-background text-center font-black" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
        ...
        <DialogFooter className="p-6 pt-4 border-t bg-muted/30 shrink-0">
          ...
          >
            {isSubmitting ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : "Save Assignment"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
