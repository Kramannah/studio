...
export const useAdminMarketingSamples = () => {
  ...
  const resetToSystemDefaults = async () => {
      try {
          ...
          await batch.commit();
          toast({ title: "Inventory Reset", description: "Successfully loaded 50 system default products." });
          return true;
      } catch (error: any) {
  ...
  return { addMarketingSamplesBulk, resetToSystemDefaults, deleteSample, updateSample, addSample };
};
